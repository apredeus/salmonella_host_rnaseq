pca12t = function (exp_table,cond_table,color_col,label_col=NULL,plot_title) {
  library(ggplot2)
  library(ggrepel)
  ...
  if (is.null(label_col)) { 
    ggplot(pcat_ann, aes(PC1, PC2)) + geom_point(aes(color = pcat_ann[,colnames(pcat_ann) %in% color_col]),size=3) + 
      ggtitle(plot_title) + labs(x=xlab,y=ylab) + scale_colour_discrete(name=color_col) + theme_bw() + 
      theme(panel.grid.major = element_line(size = 0.25))
  } else { 
    ggplot(pcat_ann, aes(PC1, PC2)) + geom_point(aes(color = pcat_ann[,colnames(pcat_ann) %in% color_col]),size=3) + 
      ggtitle(plot_title) + labs(x=xlab,y=ylab) + scale_colour_discrete(name=color_col) + theme_bw() + 
      theme(panel.grid.major = element_line(size = 0.25)) + 
      geom_text_repel(data=pcat_ann,aes(label=pcat_ann[,colnames(pcat_ann) %in% label_col]))
  }
}
