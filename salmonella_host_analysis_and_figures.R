...
setwd("~/salmonella_host_rnaseq")
source("~/salmonella_host_rnaseq/functions.R") 

################################## PART 0. QC of the mapping etc ###########################################################

cond            <- read.table("data/Conditions.tsv", row.names = 1, header = T, check.names = F, stringsAsFactors = T)
qc              <- read.table("data/bulk_qc.tsv", row.names = 1, header = T, check.names = F, stringsAsFactors = T)
qc[]            <- lapply(qc, function(x) gsub("%", "", x))
qc[]            <- lapply(qc, as.numeric)
qc              <- merge(qc, cond, by=0)
rownames(qc)    <- qc$Row.names
qc$Row.names    <- NULL

qcplot          <- qc[, colnames(qc) %in% c("Pct_uniq", "Pct_M1", "Pct_U2", "Experiment")]
qcplot$RowID    <- factor(seq_len(nrow(qcplot)))
qcplot_long     <- pivot_longer(qcplot, cols = c("Pct_uniq", "Pct_M1", "Pct_U2"), names_to = "Variable", values_to = "Value")
qcplot_long$Experiment <- as.character(qcplot_long$Experiment)
qcplot_long     <- qcplot_long %>% mutate(Variable = recode(Variable, 
                                                            "Pct_uniq" = "Unique", "Pct_M1" = "Multimapping", "Pct_U2" = "Unmapped"))
qcplot_long$Variable <- factor(qcplot_long$Variable, levels = c("Unmapped", "Multimapping", "Unique"))


ggplot(qcplot_long, aes(x = RowID, y = Value, fill = Variable)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ Experiment, scales = "free_x") +
  labs(x = "Sample", y = "Read mapping, %", title = "Unique, multimapping (~rRNA), and unmapped reads in each sample") +
  theme_bw()

colnames(qcplot_long)[3] <- "Percentage"
ql1 <- qcplot_long[qcplot_long$Experiment == "dualRNA", ]
ql2 <- qcplot_long[qcplot_long$Experiment == "haiE_host", ]

p1 <- ggplot(ql1, aes(x = RowID, y = Value, fill = Percentage)) + geom_bar(stat = "identity") + theme_bw() +
  labs(x = "Sample", y = "Read mapping, %", title = "Read mapping percentages (dualRNA-seq)") + 
  theme(plot.title = element_text(size = 18, face = "bold"), axis.title.x = element_text(size = 16),
    axis.title.y = element_text(size = 16), axis.text.x = element_text(size = 16), axis.text.y = element_text(size = 16),
    legend.title = element_text(size = 16), legend.text = element_text(size = 16))
    
p2 <- ggplot(ql2, aes(x = RowID, y = Value, fill = Percentage)) + geom_bar(stat = "identity") + theme_bw() +
  labs(x = "Sample", y = "Read mapping, %", title = "Read mapping percentages (D7795 WT/haiE)") + 
  theme(plot.title = element_text(size = 18, face = "bold"), axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16), axis.text.x = element_text(size = 16), axis.text.y = element_text(size = 16),
        legend.title = element_text(size = 16), legend.text = element_text(size = 16))
p1 + p2 + plot_layout(widths = c(1.6, 1)) + plot_layout(guides = "collect") & theme(legend.position = "right")


################################## PART1. RNA-seq differential expression analysis #########################################

salmon_exp      <- read.table("data/salmon_reads.counts.tsv", header = T, row.names = 1, check.names = F)
salmon_tpm      <- read.table("data/salmon_reads.TPM.tsv", header = T, row.names = 1, check.names = F)
...
cond            <- read.table("data/Conditions.tsv", row.names = 1, header = T, check.names = F, stringsAsFactors = T)
ann             <- salmon_exp[,c(1:3)]
exp             <- salmon_exp[,c(4:53)]
exp             <- exp[, rownames(cond)] ## reorder
sums            <- as.data.frame(colSums(exp))
colnames(sums)  <- c("Total_reads")
ggplot(sums, aes(x=rownames(sums), y=Total_reads)) +  geom_bar(stat = "identity", fill="lightblue") + theme_bw() + coord_flip()
sums            <- merge(sums, cond, by=0)
rownames(sums)  <- sums$Row.names
sums$Row.names  <- NULL 

sumsplot        <- sums[, colnames(sums) %in% c("Total_reads", "Experiment")]
sumsplot$RowID  <- factor(seq_len(nrow(sumsplot)))

sm1             <- sumsplot[sumsplot$Experiment == "dualRNA", ]
sm2             <- sumsplot[sumsplot$Experiment == "haiE_host", ]

q1 <- ggplot(sm1, aes(x = RowID, y = Total_reads)) + geom_bar(stat = "identity",  fill="lightblue") + theme_bw() +
  labs(x = "Sample", y = "Total reads", title = "Useable reads per sample (dualRNA-seq)") + 
  theme(plot.title = element_text(size = 18, face = "bold"), axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16), axis.text.x = element_text(size = 16), axis.text.y = element_text(size = 16),
        legend.title = element_text(size = 16), legend.text = element_text(size = 16)) +
  scale_y_continuous(limits = c(0,4e+08))

q2 <- ggplot(sm2, aes(x = RowID, y = Total_reads)) + geom_bar(stat = "identity",  fill="lightblue") + theme_bw() +
  labs(x = "Sample", y = "Total reads", title = "Useable reads per sample (D7795 WT/haiE)") + 
  theme(plot.title = element_text(size = 18, face = "bold"), axis.title.x = element_text(size = 16),
        axis.title.y = element_text(size = 16), axis.text.x = element_text(size = 16), axis.text.y = element_text(size = 16),
        legend.title = element_text(size = 16), legend.text = element_text(size = 16)) + 
  scale_y_continuous(limits = c(0,4e+08))

q1 + q2 + plot_layout(widths = c(1.6, 1)) + plot_layout(guides = "collect") & theme(legend.position = "right")


## make 15k table for visualization, do PCA on the rlog-transformed matrix, before and after donor correction with ComBat

exp_sorted      <- exp[order(rowMeans(exp), decreasing = T),]
exp_top15k      <- head(exp_sorted, 15000)
dds_top15k      <- DESeqDataSetFromMatrix(countData=round(exp_top15k,0), colData=cond, design = ~ Condition)
rlog_top15k     <- assay(rlog(dds_top15k, blind = T, fitType = "parametric")) 
vst_top15k      <- assay(vst(dds_top15k, blind = T, fitType = "parametric")) 

dds_top15k      <- estimateSizeFactors(dds_top15k)
dds_top15k      <- estimateDispersions(dds_top15k, fitType = "parametric")
plotDispEsts(dds_top15k) ## looks very good
boxplot(rlog_top15k)
boxplot(vst_top15k)


## that's really cool! "mean" fit is really terrible
p3 <- meanSdPlot(rlog_top15k, plot = F)$gg + theme_bw() + ggtitle("Mead SD plot - rlog, top 15k genes")
p4 <- meanSdPlot(vst_top15k, plot = F)$gg + theme_bw() + ggtitle("Mead SD plot - vst, top 15k genes")
p3 + p4

## sampleDist - kind of cool! 
library("RColorBrewer")
library("pheatmap")
sampleDists <- dist(t(vst_top15k))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- colnames(vst_top15k)
colnames(sampleDistMatrix) <- NULL
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)
pheatmap(sampleDistMatrix, clustering_distance_rows=sampleDists, clustering_distance_cols=sampleDists, col=colors)

## good ol PCA
pca12t(rlog_top15k, cond, "Condition", "Row.names", "PCA - all experiments")
pca12t(vst_top15k, cond, "Condition", "Row.names", "PCA - all experiments")

## separate PCA plots for dual and haiE

cond_dual       <- cond[cond$Experiment == "dualRNA", ]
cond_haiE       <- cond[cond$Experiment == "haiE_host", ]
exp_dual        <- exp_top15k[, colnames(exp_top15k) %in% rownames(cond_dual)]
exp_haiE        <- exp_top15k[, colnames(exp_top15k) %in% rownames(cond_haiE)]
dds_dual        <- DESeqDataSetFromMatrix(countData=round(exp_dual,0), colData=cond_dual, design = ~ Condition)
dds_haiE        <- DESeqDataSetFromMatrix(countData=round(exp_haiE,0), colData=cond_haiE, design = ~ Condition)


rlog_dual       <- assay(rlog(dds_dual, blind = T, fitType = "parametric"))
rlog_haiE       <- assay(rlog(dds_haiE, blind = T, fitType = "parametric")) 

pca12t(rlog_dual, cond_dual, "Condition", "Row.names", "PCA - dual RNA-seq only")
pca12t(rlog_haiE, cond_haiE, "Condition", "Row.names", "PCA - D7795 WT/dhaiE only")
